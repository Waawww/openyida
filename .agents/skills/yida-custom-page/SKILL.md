...
```bash
cd .Codex/skills/yida-publish/scripts && npm install
...
```bash
cd .Codex/skills/yida-publish/scripts
...
| Skill | 说明 | 用法 |
| --- | --- | --- |
| **yida-login** | 登录态管理（Cookie 持久化 + 扫码登录） | `python3 .Codex/skills/yida-login/scripts/login.py` |
| **yida-publish-page** | 编译源码 + 构建 Schema + 发布到宜搭 | `node .Codex/skills/yida-publish-page/scripts/publish.js <appType> <formUuid> <源文件路径>` |

### 编译 + 发布（一键完成）

```bash
cd .Codex/skills/yida-publish-page/scripts
npm install  # 首次需要安装依赖
node publish.js <appType> <formUuid> <源文件路径>
```
...
```bash
node .Codex/skills/yida-publish-page/scripts/babel-transform/transform.js <源文件路径>
```
