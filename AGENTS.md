...
本项目通过 AI Coding 工具（Codex/Open Code 等）+ 宜搭低代码平台，实现一句话生成完整应用。
...
# 安装 Node 依赖（首次发布前执行）
cd .Codex/skills/yida-publish-page/scripts && npm install
```
...
| 技能 | 调用命令 | 用途 |
|------|---------|------|
| `yida-login` | `python3 .Codex/skills/yida-login/scripts/login.py` | 登录态管理（通常自动触发） |
| `yida-logout` | `echo -n "" > .cache/cookies.json` | 退出登录 / 切换账号 |
| `yida-create-app` | `node .Codex/skills/yida-create-app/scripts/create-app.js "<名称>"` | 创建应用，获取 appType |
| `yida-create-page` | `node .Codex/skills/yida-create-page/scripts/create-page.js <appType> "<页面名>"` | 创建自定义页面，获取 pageId |
| `yida-create-form-page` | `node .Codex/skills/yida-create-form-page/scripts/create-form-page.js create <appType> "<表单名>" <字段JSON>` | 创建/更新表单页面 |
| `yida-get-schema` | `node .Codex/skills/get-schema/scripts/get-schema.js <appType> <formUuid>` | 获取表单 Schema，确认字段 ID |
| `yida-custom-page` | 详见 `.Codex/skills/yida-custom-page/SKILL.md` | 编写自定义页面 JSX 代码（React 16 规范、状态管理、27 个 API） |
| `yida-publish-page` | `node .Codex/skills/yida-publish-page/scripts/publish.js <appType> <formUuid> <源文件路径>` | 编译并发布自定义页面 |
...
node .Codex/skills/yida-publish-page/scripts/publish.js <appType> <formUuid> <源文件路径>
```
...
```bash
node .Codex/skills/yida-create-form-page/scripts/create-form-page.js update <appType> <formUuid> '[{"action":"add","field":{"type":"TextField","label":"新字段"}}]'
```
