...
function detectEnvironment() {
  ...
  const cwdOpenyida = path.join(process.cwd(), "openyida");
  ...
  const results = installedTools.map(({ dirName, displayName }) => {
    ...
    const workspaceRoot = isWukong
      ? path.join(home, ".real", "workspace", "openyida")
      : cwdOpenyida;
...
function run() {
  ...
  if (activeToolName && activeProjectRoot) {
    ...
    if (activeOnly.length > 0) {
      console.log(`  AI 工具:    ${activeOnly.map((r) => r.displayName).join(", ")} (活跃，但无 openyida 项目)`);
    } else {
